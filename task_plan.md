...
目标：
- 修复浏览器缩放或宽屏下后台内容固定 1520px，导致右侧出现大块空白的问题。
- 让卡片、表单和表格始终使用工作区可用宽度，并保持左右对齐。
- 完成后补充 README，并重新打包、提交 Chrome 插件。

阶段：
- 阶段 A：复现并测量工作区、内容宽度和右侧空白。状态：complete
- 阶段 B：补失败测试并改为流体宽度。状态：complete
- 阶段 C：桌面、缩放和移动端视觉验收。状态：complete
- 阶段 D：更新 README、插件包、推送和 nc48 部署。状态：complete

验收结果：
- 2048px 视口下内容右侧空白由约 216px 降为 28px 工作区内边距。
- 根 README 与插件 README 已更新。
- `nono-quick-bookmark-chrome-v0.2.1.zip` 已重新构建并推送。
- 已推送 `e5b9c7a` 并在 nc48 重建部署。

## 2026-07-15 文件夹图标弹窗选择器
状态：complete

目标：
- 将新增文件夹和行内编辑中的两排图标按钮改为单行紧凑选择器。
- 点击选择器后打开浅白磨砂弹窗，支持搜索、推荐、最近和全部图标。
- 使用 Lucide 语义图标，同时兼容已有 Emoji 图标数据。

阶段：
- 阶段 A：确认现有图标字段、FolderGlyph 和弹窗样式。状态：complete
- 阶段 B：补失败测试，定义入口和弹窗交互。状态：complete
- 阶段 C：实现共享图标注册表与选择器，并接入文件夹新增/编辑。状态：complete
- 阶段 D：全量测试、构建、推送和 nc48 部署验收。状态：complete

验收结果：
- 新增与行内编辑的图标网格已替换为紧凑选择器。
- 桌面与移动端弹窗、搜索、最近使用和选择回填均已线上验收。
- 已推送 `a7328ca` 并部署 nc48。

## 2026-07-15 后台菜单切换黑屏修复
状态：complete

目标：
- 消除后台侧栏切换页面时的黑色闪屏和近 480ms 整页淡出、淡入等待。
- 保留公开页面原有转场，不影响首页视觉效果。

根因：
- `App.vue` 对所有路由使用 `mode="out-in"` 的整页透明度动画。
- 后台工作台淡到透明时会露出 `body` 的深黑底色 `rgb(9, 10, 15)`。

结果：
- 后台路由直接替换组件，不再执行整页透明度动画；非后台路由仍保留原转场。
- 已推送 `3849f9d` 并在 nc48 重建部署。
- 线上 40ms 连续帧采样中，工作台始终存在、透明度始终为 1，控制台错误为 0。
...
目标：
- 在后台运营菜单新增“Notab 管理”。
- 仅管理顶级 Notab，支持更名、拖动排序和删除。
- 删除前展示受影响的文件夹和书签数量，并复用现有确认弹窗。
...
目标：
- 将音乐播放器默认放到首页左下方红框区域。
- 在原播放器所在的底部区域新增主题一致的“最近日程 + 未来三天”摘要卡。
- 两张卡都保留独立拖拽、尺寸和显示开关配置。

阶段：
- 阶段 A：确认现有音乐定位、卡片配置和日程数据来源。状态：complete
- 阶段 B：补失败测试，定义播放器左下定位和日程摘要数据契约。状态：complete
- 阶段 C：实现布局与磨砂摘要卡。状态：complete
- 阶段 D：全量测试、构建、推送和 nc48 部署验收。状态：complete
