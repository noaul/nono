...
export const GITHUB_CONFIG = {
	OWNER: process.env.NEXT_PUBLIC_GITHUB_OWNER || 'noaul',
	REPO: process.env.NEXT_PUBLIC_GITHUB_REPO || 'nono',
	BRANCH: process.env.NEXT_PUBLIC_GITHUB_BRANCH || 'main',
	APP_ID: process.env.NEXT_PUBLIC_GITHUB_APP_ID || '2378640',
	ROOT_PATH: process.env.NEXT_PUBLIC_GITHUB_ROOT_PATH || 'apps/blog'
} as const
```

复制 `.env.example` 中的变量到部署平台，并将 `NEXT_PUBLIC_SITE_URL` 设置为正式域名。Google Analytics 默认关闭；需要时再配置 `NEXT_PUBLIC_GOOGLE_ANALYTICS_ID`。

私钥只保留在当前页面内存中，不会写入 localStorage 或 sessionStorage。刷新页面后需要重新导入，这是为了避免公开站点持久化 GitHub App 私钥。
...
```bash
docker build \
  --build-arg NEXT_PUBLIC_SITE_URL=http://localhost:2025 \
  --build-arg NEXT_PUBLIC_NONO_URL=http://localhost:3000 \
  --build-arg NEXT_PUBLIC_GITHUB_OWNER=noaul \
  --build-arg NEXT_PUBLIC_GITHUB_REPO=nono \
  --build-arg NEXT_PUBLIC_GITHUB_BRANCH=main \
  --build-arg NEXT_PUBLIC_GITHUB_ROOT_PATH=apps/blog \
  -t nono-blog .
