...
export default function Page() {
	...
	useEffect(() => {
		void loadNodeskContent<Blogger[]>('bloggers', initialList as Blogger[]).then(items => {
			setBloggers(items)
			setOriginalBloggers(items)
		})
	}, [])
	...
	const handleDelete = (blogger: Blogger) => {
		if (confirm(copy(`确定要删除 ${blogger.name} 吗？`, `Delete ${blogger.name}?`))) {
