...
export default function CreateDialog({ blogger, onClose, onSave }: CreateDialogProps) {
	...
	const handleSubmit = () => {
		...
		onSave(formData)
		onClose()
		toast.success(blogger ? copy('更新成功', 'Updated') : copy('添加成功', 'Added'))
	}
		...
		<DialogModal open onClose={onClose} className='card w-sm'>
			...
			<div>
				<div className='mb-4 flex items-center gap-4'>
					...
					<div className='flex-1'>
						<input
							...
							placeholder={copy('博主名称', 'Blogger name')}
				...
				<textarea
					...
					placeholder={copy('博主介绍...', 'Describe the blogger…')}
			...
			<div className='mt-6 flex gap-3'>
				...
				<button onClick={handleSubmit} className='brand-btn flex-1 justify-center px-4'>
					{blogger ? copy('保存', 'Save') : copy('添加', 'Add')}
				</button>
			</div>
