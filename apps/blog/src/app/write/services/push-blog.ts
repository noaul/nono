...
export async function pushBlog(params: PushBlogParams): Promise<void> {
	...
	if (!form?.slug) throw new Error(copy('需要 slug', 'A slug is required'))

	if (mode === 'edit' && originalSlug && originalSlug !== form.slug) {
		throw new Error(copy('编辑模式下不支持修改 slug，请保持原 slug 不变', 'The slug cannot change in edit mode — keep the original'))
	}
	...
	if (allLocalImages.length > 0) {
		toast.info(copy('正在上传图片...', 'Uploading image…'))
	...
	toast.info(copy('正在创建文件...', 'Creating files…'))
