...
export function ColorConfig({ formData, setFormData }: ColorConfigProps) {
	...
	return (
		<div className='space-y-6'>
			<div>
				<label className='mb-2 block text-sm font-medium'>{copy('基础颜色', 'Base colours')}</label>
				<div className='grid grid-cols-2 gap-4'>
					<div className='flex items-center gap-3'>
						<ColorPicker value={formData.theme?.colorBrand ?? '#35bfab'} onChange={handleBrandColorChange} />
						<span className='text-xs'>{copy('主题色', 'Brand')}</span>
					</div>
					<div className='flex items-center gap-3'>
						<ColorPicker
						...
						/>
						<span className='text-xs'>{copy('次级主题色', 'Brand secondary')}</span>
					</div>
					<div className='flex items-center gap-3'>
						...
						<span className='text-xs'>{copy('主色', 'Primary')}</span>
					</div>
					<div className='flex items-center gap-3'>
						<ColorPicker
					...
					<div className='flex items-center gap-3'>
						...
						<span className='text-xs'>{copy('文章背景', 'Article background')}</span>
					</div>
				</div>
			</div>
			...
			<div>
				<div className='mb-2 flex items-center justify-between gap-3'>
					<label className='block text-sm font-medium'>{copy('背景颜色', 'Background colours')}</label>
			...
			<div className='flex flex-col gap-3'>
				{COLOR_PRESETS.map(preset => (
					<button
						...
						<span className='text-sm font-medium whitespace-nowrap'>{copy(preset.name[0], preset.name[1])}</span>
					</button>
				))}
			</div>
		</div>
