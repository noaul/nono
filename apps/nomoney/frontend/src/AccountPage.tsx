...
import { AccountAppIcon, accountTypes, buildCountryOptions, CountryFlag, getAccountTypeLabel } from './accountCatalog';
    ...
    { key: 'email', header: copy('绑定邮箱', 'Bound email'), render: (item) => <span className="break-all text-slate-500">{item.boundEmail}</span> },
    {
      ...
      render: (item) => deleteTargetId === item.id ? (
        <div className="flex min-w-[170px] items-center justify-end gap-2">
          <span className="text-xs text-danger-500">{copy('确认删除？', 'Delete?')}</span>
          <Button size="sm" variant="ghost" onClick={() => setDeleteTargetId(null)}>{copy('取消', 'Cancel')}</Button>
          <Button size="sm" variant="danger" onClick={() => remove(item)}>{copy('删除', 'Delete')}</Button>
        </div>
      ) : (
        <div className="flex justify-end gap-1">
          ...
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-xl text-slate-400 hover:bg-danger-500/10 hover:text-danger-500" onClick={() => setDeleteTargetId(item.id)} title={copy('删除', 'Delete')} aria-label={copy('删除', 'Delete')}><Trash2 size={14} /></button>
        </div>
      )
    }
  ];
    ...
    <div className="space-y-5">
          ...
          <div className="motion-list grid gap-3 md:hidden">
            {items.map((item) => (
              <article key={item.id} className="motion-card rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-ink-900">
                ...
                {deleteTargetId === item.id && <div className="mt-3 flex items-center justify-end gap-2 rounded-xl border border-danger-500/20 bg-danger-500/10 p-2"><span className="mr-auto text-xs text-danger-600 dark:text-danger-400">{copy('确认删除这个账号？', 'Delete this account?')}</span><Button size="sm" variant="ghost" onClick={() => setDeleteTargetId(null)}>{copy('取消', 'Cancel')}</Button><Button size="sm" variant="danger" onClick={() => remove(item)}>{copy('删除', 'Delete')}</Button></div>}
              </article>
            ))}
          </div>
        </>
      ...
      >
        <form id="account-form" className="space-y-5" onSubmit={submit}>
          ...
          <Field label={copy('绑定邮箱', 'Bound email')}><input className={inputClass} required type="email" value={form.boundEmail} onChange={(event) => setForm({ ...form, boundEmail: event.target.value })} placeholder="name@example.com" /></Field>
