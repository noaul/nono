...
Nono is not a single-page application. It is a collection of personal services that share one repository, one origin, and one deployment pipeline. The core Nono service provides public navigation pages, administration, authentication, and APIs. NoDesk, NoMoney, Yumi, NoStar, Clipper, and the Chrome extension cover publishing, expenses, infrastructure, GitHub Stars, saved reading, and browser capture.
...
| **Clipper** | `/clipper/*` | Web clipping, reader, full-text search, tags, and highlights | Nono Session | PostgreSQL |
| **Chrome extension** | Popup / context menu / shortcuts | Extract the current page and save a bookmark or clip | Scoped Bearer API Token | `chrome.storage.local` |
...
- Toolbar popup, context menus, and `Alt+Shift+S`, `Alt+Shift+B`, and `Alt+Shift+C` shortcuts.
...
- Captures a full article or selection through the Chrome extension and stores sanitized HTML and Markdown.
- Reader view, list and compact modes, status and starred filters, tags, and text highlights.
- Server-side refetch through the protected outbound-request layer, limited to 10 requests per 10 minutes.
- Optional bookmark association; deleting a bookmark removes the association but keeps the clip.

**Limits and current boundaries:**

- `contentHtml` and `contentMd` are limited to 2 MiB each, `sourceMeta` to 256 KiB, and the upload body to 6 MiB.
- The extension truncates on UTF-8 boundaries and marks `contentTruncated`; the server still independently enforces all limits.
- Search uses a PostgreSQL `pg_trgm` index. Queries shorter than three characters cannot use the trigram index and may fall back to a sequential scan.
- The generated `searchText` column duplicates `contentMd`; together with its GIN index, one clip may consume roughly three times its Markdown size.
- Reader content is rendered in a sandboxed `srcdoc` iframe without `allow-scripts`.
- The initial release does not include public sharing, local image mirroring, AI summaries and auto-tagging, conversion to NoDesk articles, or multiple clips linked to one bookmark.

The extension does not install persistent content scripts or retain broad host permissions. Extraction runs only when the user initiates it, and data is sent only to the configured Nono service. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for Clipper dependency notices.
...
| `/api/*` | Nono | Nono API, including NoStar and Clipper APIs |
...
| `/clipper`, `/clipper/*` | Nono / Clipper | Clipper assets and same-origin APIs |
...
| Clipper | React 18, Vite 8, Zustand, Tailwind CSS; server-side Defuddle, DOMPurify, and jsdom |
...
The root npm workspaces contain only `packages/server`, `packages/web`, and `packages/extension`. NoDesk, NoMoney, NoStar, and Clipper keep independent package managers or lockfiles under `apps/`.
...
nono/
|- packages/
|  |- server/          # Nono, NoStar, and Clipper APIs, Prisma, auth, backup, jobs
...
|  `- clipper/         # Clipper React frontend
...
### NoDesk, NoStar, and Clipper

NoDesk content is public; editing requires a Nono administrator Session. NoStar and Clipper reuse the Nono Session and redirect unauthenticated users to the Nono login page with the original destination preserved.
...
`install:all` uses five lockfile groups: the root npm workspace, NoDesk pnpm, NoMoney npm, NoStar npm, and Clipper npm.
...
| Clipper frontend | `npm run dev:clipper` | Vite, port 4175 |
...
`docker-compose.yml` passes only variables explicitly mapped under `app.environment`. Variables supported by a directly started Nono server but not mapped there, such as `CORS_ORIGIN`, `LOG_LEVEL`, or backup scheduler intervals, must also be added to the Compose environment when overridden in a container.
...
| --- | --- | --- |
| `nono_pg_data` | Nono, NoStar, Clipper, users, Sessions, Passkeys, audit, settings | PostgreSQL |
...
| Clipper | Nono Session or scoped API Token | Per-user clips and metadata |
...
| `clips:read` | Read and search clips, tags, and highlights |
| `clips:write` | Create, update, and delete clips, tags, and highlights |
| `*` | Full API Token authority |

New extension Tokens default to all five bookmark, AI, and clipping scopes. Existing Tokens are not silently expanded; add `clips:*` from the API Token page when enabling clipping on an older Token. Create one Token per device, set an expiry, and revoke it when a device is lost or retired.
...
1. Nono Server, Web, extension, NoDesk, NoMoney, NoStar, Clipper, and gateway/deployment tests.
2. NoDesk, NoStar, and Clipper type checks, plus NoStar and Clipper ESLint.
3. All production builds.
4. Playwright end-to-end tests.
5. NoStar and Clipper bundle budgets.
6. High-severity audits for the root workspace, NoDesk, NoMoney, NoStar, and Clipper.
...
Use the loopback URL that matches the deployment's `PORT`; the scripts default to `http://127.0.0.1:8188`. A failed acceptance check restores the previous image and runs acceptance again. Image rollback does not reverse database migrations or volume data.
...
Unified backups cover PostgreSQL, NoDesk files, NoMoney SQLite, and Yumi SQLite. PostgreSQL includes Nono, NoStar, Clipper, users, Passkeys, Sessions, and encrypted integration settings. Each backup creates a `.tar.gz` archive and a JSON manifest with build identity, sizes, and SHA-256 checksums.
...
The restore process validates paths, checksums, PostgreSQL TOC, and SQLite integrity; creates a safety snapshot; stops writers; restores all components; restarts the original image; and runs acceptance. A failed restore attempts to roll back to the safety snapshot.
...
The extension is under `packages/extension`; the current version is **0.4.1**.
...
Clipper dependency notices are recorded in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). This notice file does not grant a repository-wide license. The repository currently has no root `LICENSE`, so all rights are reserved unless a component states otherwise.
...
Check the exact origin, HTTPS, optional host permission, Token expiry, and required `bookmarks:read`, `bookmarks:write`, `ai:analyze`, `clips:read`, and `clips:write` scopes.
...
- Preserve the root npm, NoDesk pnpm, NoMoney npm, NoStar npm, and Clipper npm lockfiles independently.
