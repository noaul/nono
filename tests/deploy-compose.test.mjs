...
test('restores the previous image when replacing the app container fails', async () => {
  const calls = [];
  ...
  const run = async (command, args, options = {}) => {
    ...
    if (joined.includes('{{.Config.Image}}')) return { stdout: 'nono-app:0123456789ab\n' };
    if (joined.includes('backup.js create')) return { stdout: '{"id":"20260812T120000Z"}\n' };
    if (joined === 'docker compose up -d --no-deps app' && !replacementAttempted) {
      replacementAttempted = true;
      throw new Error('container replacement failed');
    }
    return { stdout: '' };
  };
  ...
  assert.equal(result.rolledBack, true);
  assert.match(result.deploymentError, /container replacement failed/);
  assert.equal(calls.some((call) => call.args.includes('--force-recreate') && call.image === 'nono-app:0123456789ab'), true);
});
