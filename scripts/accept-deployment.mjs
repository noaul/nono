...
export async function acceptDeployment({
  ...
  const htmlResponse = await fetchImpl(nostarUrl, { headers, redirect: 'follow', signal: AbortSignal.timeout(30_000) });
  ...
  const nostarAssets = await crawlJavaScriptAssets(entrySources, normalizedBaseUrl, fetchImpl, log, headers);
...
async function crawlJavaScriptAssets(entryUrls, baseUrl, fetchImpl, log, headers = {}, prefix = '/nostar/') {
